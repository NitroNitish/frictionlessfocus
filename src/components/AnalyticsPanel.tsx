...
export function AnalyticsPanel({ tasks, streakData }: AnalyticsPanelProps) {
  ...
  // Generate resistance matrix (7 days x 4 time slots) from session data
  ...
  return (
    <div className="p-5 space-y-6">
      ...
      <div className="flex rounded-xl border border-border bg-card overflow-hidden">
        <button className="flex-1 py-2.5 font-mono text-xs font-medium uppercase tracking-wider text-muted-foreground">
          Day
        </button>
        <button className="flex-1 py-2.5 font-mono text-xs font-bold uppercase tracking-wider bg-neon text-primary-foreground">
          Week
        </button>
        <button className="flex-1 py-2.5 font-mono text-xs font-medium uppercase tracking-wider text-muted-foreground">
          Month
        ...
        {stats.totalSessions > 0 && (
          <div className="mt-4 flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              Current consistency <span className="font-semibold text-neon">
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="rounded border border-neon/30 px-2 py-0.5 font-mono text-[10px] font-semibold text-neon uppercase">
            Active
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="mt-1 text-lg font-bold text-card-foreground truncate">
            {stats.mostAvoidedTask || '—'}
          </p>
          {stats.mostAvoidedCount > 0 && (
            <p className="font-mono text-[10px] font-semibold text-neon">
              +{stats.mostAvoidedCount} quits
            </p>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="font-mono text-[10px] text-muted-foreground">
                W.{String(new Date().getDate()).padStart(2, '0')}
              </span>
            </div>
          </div>
